...
public:
	...
	bool solve(int board[9][9])
	{
		...
		for(int num = 1; num <= 9; num++)
		{
				if(isValid(board, row, col, num)) // check if the number at this position will be valid, if not, back track!
				{
					board[row][col] = num;
					if(solve(board))
						return true; // everthing works now!!
					// if we are slowing the output to view the algorithm work
					if(step)
					{	
						stepsTaken++;
						std::cin.ignore();
	...
	// possible optimization here to cut out already searched positions and base the next
	// empty position off of the current position
...
int main() {
	...
	// the program will wait until you press enter			    
	s.setStepMode(true);
	...
	{
		if(s.step)
			cout << "steps taken: " << s.stepsTaken << endl;	
	  	cout << endl << "SOLUTION";
		s.displayBoard(board);
	}
	else
		cout << "there is no solution for the given board";
	return 0;
}
